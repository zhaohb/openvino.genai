...
ov_status_e ov_genai_vlm_pipeline_create_with_adapters(const char* models_path,
                                                       const char* device,
    ...
    try {
        ...
        va_end(args_ptr);
        // Same NPU-only string→int coercion as ov_genai_vlm_pipeline_create.
        if (std::string(device) == "NPU") {
            if (property.find("MAX_PROMPT_LEN") != property.end()) {
                std::string max_prompt_len = property["MAX_PROMPT_LEN"].as<std::string>();
                property.erase("MAX_PROMPT_LEN");
        ...
        // Register the LoRA adapter(s) via the ov::genai::adapters property. The
        // VLMPipeline applies them to its language model (the vision encoder is
        // left unadapted).
