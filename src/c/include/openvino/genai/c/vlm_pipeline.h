...
/**
 * @brief Initialize an ov_genai_vlm_pipeline with LoRA adapters registered at load time.
 *
 * This is equivalent to ov_genai_vlm_pipeline_create but additionally passes the
 * ov::genai::adapters property built from @p adapter_config, so the given LoRA
 * adapter(s) are applied to the pipeline's language model. (The vision encoder is
 * not adapted.) Optional plugin properties can still be provided as <key, value>
 * string pairs, exactly like ov_genai_vlm_pipeline_create.
 *
 * @param models_path Path to the directory containing the model files.
 ...
 * ov_genai_vlm_pipeline* pipe = NULL;
 * ov_genai_vlm_pipeline_create_with_adapters(model_path, "CPU", cfg, 0, &pipe);
 * @endcode
 */
OPENVINO_GENAI_C_EXPORTS ov_status_e
ov_genai_vlm_pipeline_create_with_adapters(const char* models_path,
                                           const char* device,
